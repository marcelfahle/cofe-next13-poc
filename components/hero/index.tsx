...
const Hero = ({ data }: { data: TileTasticData }) => {
  const {
    bgColor,
    bgImage,
    contentPos,
  ...
  const DivOrReference = isClickable && reference ? "div" : "div";
  ...
  const renderCopy = () => (
    <>
      {headline && (
        <h2
          className={`mb-6 ${themeToTextColor[theme]} ${
            fontSizeClass[headlineSize]
      ...
      {subhead && (
        <h3
          className={`mb-6 ${themeToTextColor[theme]} ${
            fontSizeClass[subheadSize]
          } ${isSubheadBold ? "font-bold" : ""} leading-tight`}
        >
          {subhead}
        </h3>
      )}
  ...
  const content = (
    ...
    >
      ...
      <div className="relative w-full">
        {bgImage && (
          <Image alt={headline || ""} className="w-full" media={bgImage} />
        )}
      </div>
       ...
       */}
      <div
        className={`absolute top-0 left-0 flex ${fgPosCss[contentPos]} h-full w-full overflow-hidden`}
      >
        <div
          className={`relative inline-block ${
            fgIsFullWidth ? "w-full" : ""
          } flex flex-col p-3 md:p-8 md:px-24 ${
